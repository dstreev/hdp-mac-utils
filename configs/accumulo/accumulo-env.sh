...
###
### test -z "$JAVA_HOME" && export JAVA_HOME=/usr/hdp/current/jvm/java
###
###
...
test -z "$HADOOP_PREFIX"    && export HADOOP_PREFIX=/usr/hdp/current/hadoop
test -z "$HADOOP_CONF_DIR"  && export HADOOP_CONF_DIR=/etc/hadoop/conf

test -z "$JAVA_HOME"        && export JAVA_HOME=`/usr/libexec/java_home`
test -z "$ZOOKEEPER_HOME"   && export ZOOKEEPER_HOME=/usr/hdp/current/zookeeper
test -z "$ACCUMULO_LOG_DIR" && export ACCUMULO_LOG_DIR=$HOME/var/log/accumulo
if [ -f ${ACCUMULO_CONF_DIR}/accumulo.policy ]
then
   POLICY="-Djava.security.manager -Djava.security.policy=${ACCUMULO_CONF_DIR}/accumulo.policy"
fi
test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="${POLICY} -Xmx2048m "
test -z "$ACCUMULO_MASTER_OPTS"  && export ACCUMULO_MASTER_OPTS="${POLICY} -Xmx1024m "
test -z "$ACCUMULO_MONITOR_OPTS" && export ACCUMULO_MONITOR_OPTS="${POLICY} -Xmx1024m "
test -z "$ACCUMULO_GC_OPTS"      && export ACCUMULO_GC_OPTS="-Xmx1024m "
